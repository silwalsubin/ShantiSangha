...
- Asking for JSON in the prompt without using structured outputs (`response_format: { type: "json_object" }` on OpenAI, or tool-use on Codex). Waste.
...
- **Structured outputs** (OpenAI) or **tool use** (Codex) for any extraction/classification task. Strict schema > parsing markdown.
...
- **LLM-as-judge eval** for subjective outputs (tone, groundedness). Codex Sonnet as the judge is cheap and reliable.
