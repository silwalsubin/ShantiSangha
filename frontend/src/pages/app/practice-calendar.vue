...
const practiceInfo = ref<PracticeInfo | null>(null)
...
const practiceCreatedDate = computed(() => {
  if (!practiceInfo.value) return ''
  const ca = practiceInfo.value.createdAt
  return ca.includes('T') ? ca.split('T')[0] : ca.slice(0, 10)
})

const canGoPrev = computed(() => {
  if (!practiceInfo.value) return false
  const [y, m] = practiceCreatedDate.value.split('-').map(Number)
...
const calendarDays = computed((): CalDay[] => {
  ...
  const created = practiceCreatedDate.value
...
async function loadCheckIns() {
  ...
  try {
    ...
    const items = await api.get<any[]>(`/practices/${practiceInfo.value.id}/checkins?from=${from}&to=${to}`)
...
async function toggleDay(day: CalDay) {
  if (!practiceInfo.value || day.isFuture || day.isBeforeCreation || !day.isCurrentMonth) return
  togglingDate.value = day.date
  try {
    if (day.checkin) {
      await api.delete(`/practices/${practiceInfo.value.id}/checkin?date=${day.date}`)
      checkIns.value = checkIns.value.filter(c => c.date !== day.date)
    } else {
      const result = await api.post<any>(`/practices/${practiceInfo.value.id}/checkin`, { completed: true, date: day.date })
...
async function refreshStreaks() {
  if (!practiceInfo.value) return
  try {
    const data = await api.get<any>(`/practices/${practiceInfo.value.id}?date=${todayStr.value}`)
    practiceInfo.value.currentStreak = data.currentStreak ?? data.current_streak ?? 0
    practiceInfo.value.longestStreak = data.longestStreak ?? data.longest_streak ?? 0
  } catch { /* ignore */ }
}
...
async function resetHistory() {
  ...
  try {
    await api.post(`/practices/${practiceInfo.value.id}/reset?date=${todayStr.value}`)
    const data = await api.get<any>(`/practices/${practiceInfo.value.id}?date=${todayStr.value}`)
    practiceInfo.value = {
      id: data.id,
      title: data.title,
...
async function load() {
  loading.value = true
  try {
    const data = await api.get<any>(`/practices/${practiceId.value}?date=${todayStr.value}`)
    practiceInfo.value = {
      id: data.id,
      title: data.title,
...
<template>
  <div class="mx-auto max-w-2xl space-y-5 p-4 sm:p-6">
    ...
    <template v-else-if="practiceInfo">
      <!-- Title + streaks -->
      <div class="text-center">
        <h1 class="font-serif text-lg font-bold text-sacred-text">{{ practiceInfo.title }}</h1>
        <div class="mt-2 flex justify-center gap-6">
          <div>
            <span class="text-lg font-bold text-sacred-gold">{{ practiceInfo.currentStreak }}</span>
            <span class="ml-1 text-[10px] uppercase tracking-[0.15em] text-sacred-muted">current</span>
          </div>
          <div>
            <span class="text-lg font-bold text-sacred-gold">{{ practiceInfo.longestStreak }}</span>
            ...
            <button
              ...
              :class="[
                day.isFuture || day.isBeforeCreation
                  ? 'text-sacred-text/30 cursor-default'
