...
/// Calendar view for editing check-in history of a recurring practice.
/// Navigated to from PracticeDetailView.
struct PracticeCalendarView: View {
    let practiceId: String
    let practiceTitle: String
    let practiceCreatedAt: String

    @State private var checkIns: [PracticeCheckIn] = []
                            ...
                            if day.isCurrentMonth {
                                Button {
                                    ...
                                    ZStack {
                                        ...
                                        Text("\(day.day)")
                                            .font(.system(size: 15, weight: day.checkin?.completed == true ? .semibold : .regular, design: .serif))
                                            .foregroundColor(
                                                day.isFuture || day.isBeforeCreation
                                                    ? .sacredText.opacity(0.3)
    ...
    private func load() async {
        do {
            let detail: PracticeDetail = try await api.get(practicePath)
            currentStreak = detail.currentStreak ?? 0
            longestStreak = detail.longestStreak ?? 0
        } catch {
            print("Failed to load practice: \(error)")
        }
        await loadCheckIns()
    }
    ...
    private func loadCheckIns() async {
        ...
        do {
            checkIns = try await api.get("/practices/\(practiceId)/checkins?from=\(from)&to=\(to)")
        } catch {
    ...
    private var createdDateStr: String {
        String(practiceCreatedAt.prefix(10))
    }
    ...
    private var checkinMap: [String: PracticeCheckIn] {
    ...
    private func toggleDate(_ date: String, hasCheckin: Bool) async {
        ...
        do {
            if hasCheckin {
                try await api.delete("/practices/\(practiceId)/checkin?date=\(date)")
                checkIns.removeAll { $0.date == date }
            } else {
                let body: [String: Any] = ["completed": true, "date": date]
                if let data = try? JSONSerialization.data(withJSONObject: body) {
                    let result: PracticeCheckIn = try await api.postRaw("/practices/\(practiceId)/checkin", body: data)
    ...
    private func checkAllMonth() async {
        ...
        for day in actionableDays.filter({ $0.checkin == nil }) {
            ...
            if let data = try? JSONSerialization.data(withJSONObject: body) {
                if let result: PracticeCheckIn = try? await api.postRaw("/practices/\(practiceId)/checkin", body: data) {
                    checkIns.append(result)
                }
            }
        }
    ...
    private func uncheckAllMonth() async {
        ...
        for day in actionableDays.filter({ $0.checkin != nil }) {
            try? await api.delete("/practices/\(practiceId)/checkin?date=\(day.date)")
    ...
    private func resetHistory() async {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: body) {
            let _: EmptyResponse? = try? await api.postRaw("/practices/\(practiceId)/reset?date=\(todayStr)", body: data)
        }
        let now = Date()
